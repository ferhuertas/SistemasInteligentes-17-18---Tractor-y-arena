...
public class State {
    ...
    public String toString(){
        String code="@"; 
        for (int i = 0; i < field.getF(); i++) {
            for (int j = 0; j < field.getC(); j++) {
                code+= field.getBox(i, j).getSandQuantity();
            }
        }
        code+= tractor.getX();
        code+= tractor.getY();
    ...
    public ArrayList<State> Succesors(State currentState) throws IOException {
        ...
        int ymax = currentState.getField().getC() - 1;
        int xmax = currentState.getField().getF() - 1;
    ...
    public List<Action> possibleActions(State state, ArrayList<String> myMovements, int sandToMove, ArrayList<Box> adjacent) {
        ...
        boolean existSouth = x < state.getField().getF() - 1;
        boolean existWest = y > 0;
        boolean existEast = y < state.getField().getC() - 1;
